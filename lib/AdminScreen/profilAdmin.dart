...
import 'package:project2/graphql/mutation/AdminScreen/profilAdmin.dart';
...
class ProfilAdminScreen extends StatefulWidget {
  final int idPenjual;
  const ProfilAdminScreen({super.key, required this.idPenjual});
...
class _ProfilAdminScreenState extends State<ProfilAdminScreen> {
  ...
  Future<void> updateProfilFoto(BuildContext context, String path, VoidCallback refetch) async {
    ...
    final result = await client.mutate(
      MutationOptions(
        document: gql(ProfilPenjualMutations.updateFoto),
        variables: {
    ...
    if (result.hasException) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: Text("Gagal update foto: ${result.exception.toString()}",style: const TextStyle(fontWeight: FontWeight.w700, color: Colors.white)), 
  ...
  Future<void> deleteProfilFoto(BuildContext context, VoidCallback refetch) async {
    ...
    if (!mounted) return;  // ⬅ amankan lagi
    clearImage();
  }
  ...
  Future<void> updateProfilNama(String nama, VoidCallback refetch) async {
    ...
    final result = await client.mutate(
      MutationOptions(
        document: gql(ProfilPenjualMutations.updateNama),
        variables: {
          "id_penjual": widget.idPenjual,
          "nama": nama,
        },
    ...
    if (result.hasException) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          padding: const EdgeInsets.all(8),
          content: Text("Gagal update nama: ${result.exception.toString()}",style: const TextStyle(fontWeight: FontWeight.w700, color: Colors.white)), 
          backgroundColor: const Color.fromARGB(202, 0, 0, 0)
        ),
      );
    } else {
  ...
  Future<void> updateProfilTelepon(String telepon, VoidCallback refetch) async {
    ...
    final result = await client.mutate(
      MutationOptions(
        document: gql(ProfilPenjualMutations.updateTelepon),
        variables: {
          "id_penjual": widget.idPenjual,
          "telepon": telepon,
        },
    ...
    if (result.hasException) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          padding: const EdgeInsets.all(8),
          content: Text("Gagal update nomor telepn: ${result.exception.toString()}",style: const TextStyle(fontWeight: FontWeight.w700, color: Colors.white)), 
          backgroundColor: const Color.fromARGB(202, 0, 0, 0)
        ),
      );
    } else {
  ...
  Future<void> updateProfilPassword(String newPassword, String oldPassword,VoidCallback refetch) async {
    ...
    final result = await client.mutate(
      MutationOptions(
        document: gql(ProfilPenjualMutations.updatePassword),
        variables: {
          "id_penjual": widget.idPenjual,
    ...
    return Scaffold(
      ...
      body: Query(
        ...
        builder: (QueryResult result, {fetchMore, refetch}) {
          ...
          final penjual = result.data?['penjualsbyid'];
          if (penjual == null) {
            return Center(
              child: Container(
                color: const Color.fromARGB(202, 0, 0, 0),
                padding: const EdgeInsets.all(8),
                child: Text("Penjual tidak ada: ${result.exception.toString()}",style: const TextStyle(fontWeight: FontWeight.w700, color: Colors.white))
              )
            );
          }
          return SafeArea(
                                  ...
                                  Stack(
                                    ...
                                    children: [
                                      Container(
                                        ...
                                        child: ClipOval(
                                          child: (penjual['profil'] != null && penjual['profil'] != '')
                                            ? Image.file(
                                                File(penjual['profil']),
                                                fit: BoxFit.cover,
                                                width: 200,
                                            ...
                                            showModalBottomSheet(
                                              ...
                                              builder: (context) {
                                                return FractionallySizedBox (
                                                  ...
                                                  widthFactor: 1.0,
                                                  heightFactor: (penjual['profil'] != null && penjual['profil'] != '') ? 0.415 : 0.3,
                                                  child: Padding(
                                                    padding: const EdgeInsets.all(10),
                                                    child: Column(
                                                      crossAxisAlignment: CrossAxisAlignment.start,
                                                      children: [
                                                        if (penjual['profil'] != null && penjual['profil'] != '')
                                                          Padding(
